...
- (void)extractSQL:(NSString *)sql argumentsList:(va_list)args intoString:(NSMutableString *)cleanedSQL arguments:(NSMutableArray *)arguments {
    ...
    for (NSUInteger i = 0; i < length; ++i) {
        ...
        if (last == '%') {
            switch (current) {
                case '@':
                    arg = __builtin_va_arg(args, id);
                    break;
                case 'c':
                    // warning: second argument to '__builtin_va_arg' is of promotable type 'char'; this __builtin_va_arg has undefined behavior because arguments will be promoted to 'int'
                    arg = [NSString stringWithFormat:@"%c", __builtin_va_arg(args, int)];
                    break;
                case 's':
                    arg = [NSString stringWithUTF8String:__builtin_va_arg(args, char*)];
                    break;
                case 'd':
                case 'D':
                case 'i':
                    arg = [NSNumber numberWithInt:__builtin_va_arg(args, int)];
                    break;
                case 'u':
                case 'U':
                    arg = [NSNumber numberWithUnsignedInt:__builtin_va_arg(args, unsigned int)];
                    break;
                case 'h':
                    i++;
                    if (i < length && [sql characterAtIndex:i] == 'i') {
                        //  warning: second argument to '__builtin_va_arg' is of promotable type 'short'; this __builtin_va_arg has undefined behavior because arguments will be promoted to 'int'
                        arg = [NSNumber numberWithShort:(short)(__builtin_va_arg(args, int))];
                    }
                    else if (i < length && [sql characterAtIndex:i] == 'u') {
                        // warning: second argument to '__builtin_va_arg' is of promotable type 'unsigned short'; this __builtin_va_arg has undefined behavior because arguments will be promoted to 'int'
                        arg = [NSNumber numberWithUnsignedShort:(unsigned short)(__builtin_va_arg(args, uint))];
                    }
                    else {
                        i--;
                    }
                ...
                case 'q':
                    i++;
                    if (i < length && [sql characterAtIndex:i] == 'i') {
                        arg = [NSNumber numberWithLongLong:__builtin_va_arg(args, long long)];
                    }
                    else if (i < length && [sql characterAtIndex:i] == 'u') {
                        arg = [NSNumber numberWithUnsignedLongLong:__builtin_va_arg(args, unsigned long long)];
                    }
                    else {
                        i--;
                    }
                    break;
                case 'f':
                    arg = [NSNumber numberWithDouble:__builtin_va_arg(args, double)];
                    break;
                case 'g':
                    // warning: second argument to '__builtin_va_arg' is of promotable type 'float'; this __builtin_va_arg has undefined behavior because arguments will be promoted to 'double'
                    arg = [NSNumber numberWithFloat:(float)(__builtin_va_arg(args, double))];
                    break;
                case 'l':
                    i++;
                    if (i < length) {
                        ...
                        if (next == 'l') {
                            ...
                            //%ld
                            arg = [NSNumber numberWithLong:__builtin_va_arg(args, long)];
                        }
                        else if (next == 'u') {
                            //%lu
                            arg = [NSNumber numberWithUnsignedLong:__builtin_va_arg(args, unsigned long)];
                        }
                        else {
                            i--;
                        }
...
- (BOOL)update:(NSString*)sql withErrorAndBindings:(NSError**)outErr, ... {
    va_list args;
    __builtin_va_start(args, outErr);
