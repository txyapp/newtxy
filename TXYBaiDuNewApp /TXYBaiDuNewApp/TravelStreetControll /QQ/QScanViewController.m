...
<UITableViewDataSource,UITableViewDelegate,UIPickerViewDataSource,UIPickerViewDelegate,UIAlertViewDelegate,QMSSearchDelegate>
{
    int mark;
    UIButton *saveB;
    //type图片方式
...
@property (nonatomic, strong) QMSWalkingRouteSearchOption *walkingRouteSearchOption;
@property (nonatomic, strong) QMSDrivingRouteSearchOption *drivingRouteSearchOption;
@property (nonatomic, strong) QMSBusingRouteSearchOption *busingRouteSearchOption;
@end
...
//        a.latitude = [((XuandianModel *)_dataForTab[i]).latitudeNum doubleValue];
//        a.longitude = [((XuandianModel *)_dataForTab[i]).longitudeNum doubleValue];
//        ((XuandianModel *)_dataForTab[i]).currentLocation = a;
//    }
...
-(void) pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
   ...
   NSString *hh= [label0.text stringByReplacingOccurrencesOfString:@"时" withString:@""];
    ...
    //NSLog(@"%@ %@ %@",label0.text,label6.text,label7.text);
    ...
    [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status)
     {
         ...
         if (single <= newDataForTab.count -1)
         {
             ...
             model.latitude =([[FireToGps sharedIntances]hhTrans_bdGPS:currentCll]).latitude;
             model.longtitude = ([[FireToGps sharedIntances]hhTrans_bdGPS:currentCll]).longitude;
             model.x = ((XuandianModel *)newDataForTab[single]).x;
             model.y = ((XuandianModel *)newDataForTab[single]).y;
             model.ScanRate = rate;
...
{
    NSLog(@"start2Dlo :%lf  start2Dla :%lf",start2D.longitude,start2D.latitude);
    if (tripType == 0) {
        //步行
        self.walkingRouteSearchOption = [[QMSWalkingRouteSearchOption alloc] init];
        [self.walkingRouteSearchOption setFromCoordinate:start2D];
        [self.walkingRouteSearchOption setToCoordinate:end2D];
        [self.mapSearcher searchWithWalkingRouteSearchOption:self.walkingRouteSearchOption];
    }
    if (tripType == 1) {
        //驾车
        self.drivingRouteSearchOption = [[QMSDrivingRouteSearchOption alloc] init];
        [self.drivingRouteSearchOption setFromCoordinate:start2D];
        [self.drivingRouteSearchOption setToCoordinate:end2D];
...
- (void)searchWithWalkingRouteSearchOption:(QMSWalkingRouteSearchOption *)walkingRouteSearchOption didRecevieResult:(QMSWalkingRouteSearchResult *)walkingRouteSearchResult
{
    ...
    [self DealWithLinesData:walkingRoutePlan withCount:count];
//    NSLog(@"Walking result:%@", walkingRouteSearchResult);
//    NSLog(@"walkingRoutePlan is :%@",walkingRoutePlan.steps);
}
//驾车算路回调
- (void)searchWithDrivingRouteSearchOption:(QMSDrivingRouteSearchOption *)drivingRouteSearchOption didRecevieResult:(QMSDrivingRouteSearchResult *)drivingRouteSearchResult
{
   // NSLog(@"driving result:%@", drivingRouteSearchResult);
    QMSRoutePlan *walkingRoutePlan = [[drivingRouteSearchResult routes] firstObject];
    NSUInteger count = walkingRoutePlan.polyline.count;
    [self DealWithLinesData:walkingRoutePlan withCount:count];
}
-(void)DealWithLinesData:(QMSRoutePlan *)route withCount:(int)count
{
    NSMutableArray *curentArr = [NSMutableArray arrayWithCapacity:0];
    for (QMSRouteStep *step in route.steps) {
        [curentArr addObject:step.polyline_idx[0]];
        [curentArr addObject:step.polyline_idx[1]];
    }
    NSSet *lastPoints = [NSSet setWithArray:curentArr];
    NSLog(@"%@",lastPoints);
    NSMutableArray *waitePoints = [NSMutableArray arrayWithArray:[lastPoints allObjects]];
    if (count >0) {
        for(int i = 0 ; i < count ; i++){
            CLLocationCoordinate2D coordinate;
            [[route.polyline objectAtIndex:i] getValue:&coordinate];
            DBResultModel model;
            model.latitude =[[FireToGps sharedIntances]hhTrans_bdGPS:coordinate].latitude;
            model.longtitude = [[FireToGps sharedIntances]hhTrans_bdGPS:coordinate].longitude;
            model.x = MKMapPointForCoordinate(coordinate).x;
            model.y = MKMapPointForCoordinate(coordinate).y;
            model.ScanRate = rate;
            model.isState = isState;
            model.isCycle = isCycle;
            model.len = speed/3.6*rate;
            ...
            if ([waitePoints containsObject:[NSNumber numberWithInt:i]]) {
                model.isWaitPoint = 1;
            }
...
//                        }
//                    }
                    //判断是那一个点暂停的，除了首末两个选中的点，其他店只判断第二个点。也就是只暂停conut-1 避免暂停两次
    ...
    if (_linesData.count >0) {
        ...
        else
        {
            for (int i = 0; i<_dataForTab.count; i++) {
                ...
                ((XuandianModel *)_dataForTab[i]).x = MKMapPointForCoordinate(a).x;
                ((XuandianModel *)_dataForTab[i]).y = MKMapPointForCoordinate(a).y;
        ...
        {
            [maptypeManager saveFileRecordwithWhichApp:self.boundle withWhichMap:@"tencent"];
        }
    }
    else
    {
...
{
    model.currentLocation =[[FireToGps sharedIntances]gcj02Encrypt:[model.latitudeNum doubleValue] bdLon:[model.longitudeNum doubleValue]] ;
